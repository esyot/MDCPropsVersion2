    ...
    <div class="flex flex-grow overflow-hidden">
                            ...
                            <div class="flex justify-between space-x-2 items-center">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <div>
                                        ...
                                        <h1 id="message-latest" class="w-[250px] truncate text-gray-600">
                                            {{ $contact->content }}</h1>
                                    </div>
                                </div>
        ...
        <div class="lg:w-3/4 flex flex-col">
            ...
            <div id="messages-container"
                class="flex flex-col flex-grow overflow-y-auto p-4 bg-blue-300 custom-scrollbar">
                @foreach($allMessages as $message)
                    <div
                        class="relative space-x-2 flex {{ $message->sender_name == $current_user_name ? 'justify-end' : 'justify-start' }} space-x-0 p-3 rounded-lg group">
                        ...
                        <div class="space-y-2">
                            @if($message->replied_message != null)
                                ...
                                <div class="mt-20">
                                    <div class="relative w-[400px]">
                                        <div
                                            class="absolute bottom-[-10px] left-0 w-full bg-opacity-20 py-2 px-3 text-white bg-gray-400 rounded-tl-full rounded-tr-full rounded-s-full mt-12">
                            ...
                            <div onmouseover="document.getElementById('icons-{{$message->id}}').classList.remove('hidden')"
                                ...
                                @if($message->sender_name == $current_user_name)
                                    <div id="icons-{{$message->id}}" class="items-center hidden">
                                        <form class="flex" action="{{ route('messageReacted', ['id' => $message->id]) }}">
                                            <button type="submit" title="reply to this message"
                                                class="px-1 py-1.2 rounded-full hover:bg-gray-300">
                                                <i title="add a reaction"
                                                    class="hover:text-gray-500 fa-regular fa-face-smile"></i>
                                            </button>
                                            <button
                                                onclick="handleButtonClick('{{$message->id}}', '{{$sender_name}}', '{{$receiver_name}}')"
                                                type="button" title="reply to this message"
                                                class="px-1 py-1.2 rounded-full hover:bg-gray-300">
                                                <i class="hover:text-gray-500 fa-solid fa-share"
                                ...
                                @endif
                                <div
                                    class="{{ $message->sender_name == $current_user_name ? 'bg-blue-500 text-white' : 'bg-white' }} text-justify inline-block max-w-full p-4 rounded-2xl shadow-sm relative">
            ...
            @foreach($allMessages as $message)
                <div id="message-{{$message->id}}" class="p-2 bg-gray-200 bg-opacity-50 hidden">
                    <div class="flex justify-between">
                        ...
                        <button class="text-2xl hover:text-white" onclick="document.getElementById('message-{{$message->id}}').classList.add('hidden'); 
                    ...
                    <div class="flex space-x-2">
                        <form action="{{ route('messageSend') }}" method="POST"
                            ...
                            <input type="text" name="content" placeholder="Aa"
                                class="block w-full px-3 py-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400 transition-colors">
                            <button type="submit"
                                class="w-10 h-10 rounded-full bg-teal-500 text-white hover:bg-teal-600 transition-colors">
