        ...
        <div class="flex items-center space-x-1">
            <div class="flex items-center space-x-1">
                ...
                <a hx-get="{{ route('admin.calendar-move', ['category' => $currentCategory->id, 'action' => 'left', 'year' => $currentDate->format('Y'), 'month' => $currentDate->format('m')]) }}"
                    ...
                    <i
                        class="fas fa-chevron-circle-left fa-2xl {{ $setting->darkMode ? 'text-gray-100' : 'text-blue-500 ' }} hover:opacity-50"></i>
                </a>
    ...
    <div id="calendar-content" class="grid grid-cols-4 gap-1 m-1">
        ...
        @foreach(range(1, 12) as $month)
                ...
                <div id="calendar-months" title="Click to select month"
                    ...
                    class="hover:opacity-50 rounded-t-xl bg-white shadow-md border cursor-pointer {{$setting->transition ? 'transition-transform duration-300 ease-in-out hover:scale-90' : '' }}">

                    <div
                        class="text-center {{ $setting->darkMode ? 'bg-gray-500 text-white' : 'bg-blue-600 text-blue-100'  }}  rounded-t-xl">
                    ...
                    <div id="calendar-month-content" class="grid grid-cols-7">
                        ...
                        @for ($day = 1; $day <= $daysInMonth; $day++)
                                    ...
                                    <div
                                        class="{{ $hasRecord ? 'bg-blue-300 text-blue-100 border border-gray-100' : '' }} 
                                        ...
                                        <span class="relative inline-block">
                                            @if(\Carbon\Carbon::now()->isToday() && \Carbon\Carbon::now()->day == $day && $monthStart->month == \Carbon\Carbon::now()->month && $currentYear == \Carbon\Carbon::now()->format('Y'))
                                                <i id="today-circle-icon"
                                                    class="fas fa-circle text-green-500 text-[4px] absolute top-5 left-0 right-0 z-50"></i>
                                            @endif
                                            <span>{{ $day }}</span>
                                        </span>
                                    </div>
