<div class="bg-white rounded shadow-md">
    ...
    <div class="flex justify-end space-x-1 p-2 bg-gray-100 rounded-b">
        <button
            onclick="document.getElementById('reserved-items-to-return-modal-{{$transaction_id}}').classList.add('hidden')"
            class="px-4 p-2 border border-blue-300 hover:opacity-50 text-blue-500 rounded">
            Close
        </button>
        @if($reservation->returned_at == null)
            <a href="{{ route('admin.reserved-items-returned', ['transaction_id' => $transaction_id]) }}"
