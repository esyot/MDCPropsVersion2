...
@foreach($contacts as $contact)
                <a href="{{ route('chatSelected', [ 'contact'=> $contact->sender_name ]) }}">
                    <li class="{{ $contact->isRead == false ? 'font-bold bg-gray-400 text-black' : '' }} hover:bg-gray-300 p-3 rounded-lg mb-2 cursor-pointer duration-300">
                        <div class="flex justify-between mx-2 items-center">
                            <div class="flex items-center space-x-2">
                                ...
                                <div>
                                    ...
                                    <h1 class="{{ $contact->isRead == false ? 'font-bold' : '' }} w-[150px] truncate">{{ $contact->content }}</h1>
                                </div>
                            </div>
