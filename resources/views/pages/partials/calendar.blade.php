    ...
    <div class="flex items-center space-x-2">
        <form id="filter-form" class="flex justify-around space-x-4" action="{{ route('dateCustom')}}" method="GET">
            ...
            @foreach($currentCategory as $category)
                <a title="Today"
                    hx-get="{{ route('calendarMove', ['action' => 'today', 'category' => $category->id, 'year' => $currentDate->format('Y'), 'month' => $currentDate->format('n')])}}"
            ...
            @foreach($currentCategory as $category)
                <a title="Slide to left"
                    hx-get="{{ route('calendarMove', ['action' => 'left', 'category' => $category->id, 'year' => $currentDate->format('Y'), 'month' => $currentDate->format('n')])}}"
                    hx-trigger="click" hx-swap="innerHTML" hx-target="#dashboard">
            ...
            @foreach($currentCategory as $category)
                <a title="Slide to right"
                    hx-get="{{ route('calendarMove', ['action' => 'right', 'category' => $category->id, 'year' => $currentDate->format('Y'), 'month' => $currentDate->format('n')])}}"
                    hx-trigger="click" hx-swap="innerHTML" hx-target="#dashboard">
                    <i
